...
class PatternTopbar extends HTMLElement {
  connectedCallback() {
          ...
          ? '<div class="right">' +
              ...
              '<div class="avatar-wrap">' +
                ...
                '<div class="avatar-menu" id="av-menu" role="menu">' +
                  ...
                  '<a href="#" class="menu-item" role="menuitem"><i class="ti ti-logout menu-icon"></i>Logout</a>' +
                '</div>' +
              '</div>' +
            '</div>'
          : '') +
    ...
    if (!isAuth) {
      /* hamburger → dispatch nav-toggle so pages can push in their left nav */
      var hamburger = shadow.querySelector('.icon-btn[aria-label="Open menu"]');
      if (hamburger) {
