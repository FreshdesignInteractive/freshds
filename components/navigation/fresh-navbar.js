/* ============================================================
   ...
   Events:
     ...
     profile-click   , fired when Profile is selected from the avatar menu
     settings-click  , fired when Settings is selected from the avatar menu
     signout-click   , fired when Sign out is selected from the avatar menu
   ============================================================ */
...
class FreshNavbar extends HTMLElement {
  ...
  constructor() {
    ...
    this._outsideClick = (e) => {
      if (!this.contains(e.target)) this._closeAvatarMenu();
    };
    this._escHandler = (e) => {
      if (e.key === 'Escape' && this._menuOpen) this._closeAvatarMenu();
    };
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .avatar-menu-items { padding: var(--space-1) 0; }
        ...
        .avatar-menu-item .ti { font-size: 15px; flex-shrink: 0; }
        ...
        .avatar-menu-item--danger { color: var(--danger-6); }
        .avatar-menu-item--danger:hover { background: var(--danger-1); color: var(--danger-7); }
      </style>
      ...
      <header class="bar" part="bar">
        ...
        <div class="avatar-menu" part="avatar-menu" hidden>
          <div class="avatar-menu-header">
            <div class="avatar-menu-name"></div>
          </div>
          <div class="avatar-menu-items">
            <button class="avatar-menu-item" data-action="profile"><i class="ti ti-user-circle"></i>Profile</button>
            <button class="avatar-menu-item" data-action="settings"><i class="ti ti-settings"></i>Settings</button>
            <div class="avatar-menu-divider"></div>
            <button class="avatar-menu-item avatar-menu-item--danger" data-action="signout"><i class="ti ti-logout"></i>Sign out</button>
          </div>
        </div>
      </header>
    `;
    ...
    if (this._menuOpen) {
      this.shadowRoot.querySelector('.avatar-menu').hidden = false;
    }
  }
  ...
  _handleHostClick(e) {
    ...
    const isAvatar = path.some(el => el instanceof Element && el.tagName === 'FRESH-AVATAR');
    if (isAvatar) {
      this._toggleAvatarMenu();
      return;
    }

    // Menu item click (inside shadow DOM — use composedPath)
    const actionEl = path.find(el => el instanceof Element && el.dataset && el.dataset.action);
    if (actionEl) {
      this._closeAvatarMenu();
      this.dispatchEvent(new CustomEvent(actionEl.dataset.action + '-click', { bubbles: true, composed: true }));
    }
  }

  _toggleAvatarMenu() {
    this._menuOpen ? this._closeAvatarMenu() : this._openAvatarMenu();
  }

  _openAvatarMenu() {
    this._menuOpen = true;
    const menu = this.shadowRoot.querySelector('.avatar-menu');
    if (!menu) return;
    // Populate name from slotted fresh-avatar
    const av = this.querySelector('fresh-avatar');
    const nameEl = menu.querySelector('.avatar-menu-name');
    if (nameEl) nameEl.textContent = av?.getAttribute('name') || '';
    menu.hidden = false;
    // Defer outside-click listener so this click doesn't immediately close the menu
    setTimeout(() => {
